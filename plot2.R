...
doPlot <- function(x) {
  ...
  plot(powerDF$Global_active_power ~ powerDF$Datetime
       , type="l"
       , xlab=""
       , ylab="Global Active Power (kilowatts)")         # Generating the graphic...
  message("Copying plot to PNG file format...")          # More messaging...
  dev.copy(png, file="plot2.png", width=480, height=480) # Copying plot to PNG file format...
  dev.off()                                              # Disconnecting from PNG output device...
  message("******************************************************************************")
  message("File Plot2.png saved in ", getwd())           # Message saying where I've placed the PNG file.
  message("******************************************************************************")
